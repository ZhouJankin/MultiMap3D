...
mpPclGlobalMap(new PointCloudMapping::PointCloud()), mpPclObstacle(new PointCloud()), mpPclGroundPlane(new PointCloud()),mpPclKeyframeCloud(new PointCloud()), 
...
//                mViewer.showCloud(pPlane);
        ...
        //update 传递keyframe_cloud (这里传递的其实是N个关键帧合成的一个点云，并不是1个关键帧的点云)
        //*mpPclKeyframeCloud = *pNewCloud;
        {
            unique_lock<mutex> lock(mMtxKeyframeCloudUpdated);
            mpPclKeyframeCloud = pNewCloud->makeShared();
        }
...
{
    //update plysave
    pcl::io::savePLYFile("my_demo_result.ply", *mpPclGlobalMap);
	pcl::io::savePCDFile( "my_demo_result.pcd", *mpPclGlobalMap );
...
bool PointCloudMapping::getKeyframeCloud(PointCloud::Ptr &pCloud)
{
    unique_lock<mutex> lock(mMtxKeyframeCloudUpdated);
    if (mpPclKeyframeCloud->empty())
        return false;
    pCloud = mpPclKeyframeCloud->makeShared();
    return true;
}
