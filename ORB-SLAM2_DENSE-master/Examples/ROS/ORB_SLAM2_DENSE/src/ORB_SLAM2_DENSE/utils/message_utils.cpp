...
namespace ORB_SLAM2_DENSE
{
    ...
    private_nh_("~"), listener_(listener), mpSystem_(pSystem), pcl_map_(new PointCloudMapping::PointCloud()), pcl_keyframeMap_(new PointCloudMapping::PointCloud()), 
    pcl_plane_(new PointCloudMapping::PointCloud())
    {
        ...
        // initalize publisher and subscriber 建立轨迹，相机位姿，点云的发布节点
...
//            mpPclMapper_->getGlobalCloud(pcl_map_);
            ...
            //update
            mpPclMapper_->getKeyframeCloud(pcl_keyframeMap_);
        }
        else
        {
            //取出全局点云mpPclGlobalMap 放入pcl_map_
            mpPclMapper_->getGlobalCloud(pcl_map_);
            //update 取得关键帧点云
            mpPclMapper_->getKeyframeCloud(pcl_keyframeMap_);
        }
        ...
        //update 干嘛用的？
        pcl_keyframeMap_->width = pcl_keyframeMap_->size();
...
//            ROS_WARN_STREAM("map<--odom transform:\n" << mTmo);
            ...
            //update 
            pcl::transformPointCloud(*pcl_keyframeMap_, *pcl_keyframeMap_, mTmo);
        }
        else if (use_tf_)
        {
            tf::Transform Tmo;
...
//        ROS_WARN("PCL Message height: %d, width: %d, size: %d", pcl2_msgs.height, pcl2_msgs.width, pcl2_msgs.data.size());
        ...
        cout<<"debug in message_utils, pcl2 publish complete"<<endl;
        ...
        //update
        // sensor_msgs::keyframe_Cloud_msgs
        sensor_msgs::PointCloud2 keyframe_Cloud_msgs;
        pcl::toROSMsg(*pcl_keyframeMap_, keyframe_Cloud_msgs);
        keyframe_Cloud_msgs.header.stamp = ros::Time::now();
        keyframe_Cloud_msgs.header.frame_id = map_frame_;
        keyframe_pub_.publish(keyframe_Cloud_msgs);
        cout<<"debug in message_utils, keyframe_map publish complete"<<endl;
